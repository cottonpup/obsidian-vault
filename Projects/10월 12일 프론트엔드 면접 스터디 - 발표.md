...
# 비동기 함수
비동기 함수는 이벤트 루프를 통해 비동기적으로 작동하는 함수입니다. 비동기적이란 쉽게 말해 함수 실행을 특정 코드가 끝날 때 까지 기다리지 않고 다음 코드를 먼저 실행하는 것을 의미 합니다. 동기 함수는 스레드를 멈추는 블록킹 현상이 있는데 비동기 함수는 별도의 스레드로 동시에 여러 작업을 실행하게 합니다. 

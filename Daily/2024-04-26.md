...
#### 데이터를 요청하고 응답을 받는 방법?
아래 URL 에서 `${from}`에는 환전 기준 통화를 `${to}` 는 환전 대상 통화를 넣어서 요청합니다.
```
https://cdn.jsdelivr.net/gh/fawazahmed0/currency-api@{date}/{apiVersion}/{endpoint}/currencies/{from}/{to}.json
```
성공적인 요청을 보냈을 때, 응답형식은 JSON 형태로 반환됩니다. 

...
- Action: just an object with an action type type and a payload
- Action creator: a function that returns an action
- Reducers: another function that changes the state, it checks the action type and then uses the action payload to modify the state
- Action type: is THE KEY, its THE HINT for the reducer because the reducer needs to see the action.type to know what to do with the payload
